...
(function () {
    ...
    function spin(n, a, i, f) {
        const p = document.getElementById(n + '-prev'), c = document.getElementById(n + '-val'), x = document.getElementById(n + '-next');
        if (p) p.textContent = i > 0 ? f(a[i - 1]) : ''; if (c) c.textContent = f(a[i]); if (x) x.textContent = i < a.length - 1 ? f(a[i + 1]) : '';
    }
    function kelvinToTint(k) {
        const d = (k - 5500) / 2500; if (Math.abs(d) < 0.05) return { color: 'transparent', opacity: 0 };
